...
$string['customwelcomemessage_help'] = 'Принимаемые форматы: обычный текст или авто-формат Moodle, может включать HTML-теги и мультиязычные теги, а также следующие подстановки:
<br>
* Название курса {$a->coursename}<br>
* Ссылка на страницу профиля пользователя {$a->profileurl</br>.
* Адрес электронной почты пользователя {$a->email<br>
* Полное имя пользователя {$a->fullname}<br>
* Имя пользователя {$a->firstname}<br>
* Фамилия пользователя {$a->lastname}<br>
* Роль пользователя в курсе {$a->courserole}<br>';
