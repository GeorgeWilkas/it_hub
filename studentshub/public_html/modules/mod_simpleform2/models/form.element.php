...
class simpleForm2Element extends simpleForm2BaseModel {
    ...
    public $nonHtmlParams = array('label', 'regex', 'error', 'send-in-email', 'help', 'recipient-element-name', 'layout');
    ...
    public function parse($code) {
        ...
        if (!is_null($this->getParam('value')))
            $this->values[] = $this->getParam('value');
        ...
        if ($this->form->getStyle() == 'compact' && $this->getParam('placeholder', '') == '') {
            $this->setParam('placeholder', $this->getParam('label', ''));
        }

        $this->code = preg_replace('~{element index="\d+" ~', '{element ', $this->code);
    ...
    public function validate() {
        if (!$this->isRequired && empty($this->inputValue)) {
            return true;
        }
    ...
    public function onAfterSendForm() {
